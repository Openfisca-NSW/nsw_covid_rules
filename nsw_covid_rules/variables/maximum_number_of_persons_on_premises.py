...
class maximum_number_of_persons_general_area(Variable):
    ...
    def formula(buildings, period, parameters):
        ...
        in_greater_sydney = buildings('in_Greater_Sydney', period)
        exempted_gathering = buildings('exempted_gathering', period)
        ExemptedGathering = (exempted_gathering.possible_values)
        is_place_of_residence = (
                                 (type_of_premises == TypeOfPremises.place_of_residence)
                                 )
        is_holiday_home_or_short_term_rental = (
                                                (type_of_premises == TypeOfPremises.holiday_home)
                                                +
                                                (type_of_premises == TypeOfPremises.short_term_rental)
                                                )
        is_entertainment_facility = (
                                     (type_of_premises == TypeOfPremises.entertainment_facility)
                                     )
        is_major_recreation_facility = (
                                        (type_of_premises == TypeOfPremises.major_recreation_facility)
                                        )
        is_group_gym_or_dance_class = (
                                       (type_of_premises == TypeOfPremises.indoor_recreation_facility)
        ...
        is_commercial_vessel = (
                                (type_of_premises == TypeOfPremises.commercial_scuba_diving_vessel)
                                 +
        ...
        is_Greater_Sydney_construction_site = (
                                               (type_of_premises == TypeOfPremises.construction_site)
                                               *
                                               buildings('in_Greater_Sydney', period)
                                               )
        ...
        return np.select(
                         ...
                         [
                          buildings('maximum_number_of_persons_in_general_area_residence', period),
                          buildings('maximum_number_of_persons_in_holiday_home_or_short_term_rental_general_area', period),
                          buildings('maximum_number_of_persons_in_general_area_entertainment_facilities', period),
                          buildings('maximum_number_of_persons_in_general_area_major_recreation_facility', period),
                          buildings('maximum_number_of_persons_in_gym_group_class', period),
                          ...
                          buildings('maximum_number_of_persons_on_general_area_general_premises', period)
                         ]
                        )
...
class maximum_number_of_persons_on_general_area_general_premises(Variable):
    ...
    def formula(buildings, period, parameters):
        size_of_premises = buildings('size_of_premises', period)
...
class maximum_number_of_persons_in_general_area_entertainment_facilities(Variable):
    ...
    def formula(buildings, period, parameters):
        size_of_premises = buildings('size_of_premises', period)
        allowable_fixed_seating_capacity = buildings('fixed_seating_capacity', period) / 2 # to get 50% of capacity
...
class maximum_number_of_persons_in_general_area_major_recreation_facility(Variable):
    ...
    def formula(buildings, period, parameters):
        size_of_premises = buildings('size_of_premises', period)
        allowable_fixed_seating_capacity = buildings('fixed_seating_capacity', period) / 2 # to get 50% of capacity
        unfixed_seating_area = buildings('size_of_unfixed_seating_area', period)
